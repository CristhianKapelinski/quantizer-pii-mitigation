...
QUICK_DESC="one cell end-to-end on a 16 GB GPU (fine-tune ~85 min measured) -- reduced Claim-1 check"
...
Usage:
  bash reproduce.sh                 # run ALL experiments, in order
  bash reproduce.sh <name> [<name>] # run only the named experiment(s)
  bash reproduce.sh quick           # reduced check: one cell end-to-end (16 GB GPU)
  bash reproduce.sh --list          # list experiment names + descriptions
